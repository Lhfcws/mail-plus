 ...
 * @time 14-10-23.
 */
public class POP3ProtocolReceiver implements POP3Receiver {

    private static Log LOG = LogFactory.getLog(POP3ProtocolReceiver.class);
    private CommonSocket socket;
    private ReceiveRequest req;

    public POP3ProtocolReceiver(ReceiveRequest req) {
        this.socket = new CommonSocket();
        this.req = req;
    }

    public void setLatestMailID(String id) {
//        this.lastID = id;
    }
    ...
    private List<Email> receiveData() throws Exception {
        socketSend(
                ProtocolConsts.LIST, ""
        );
...
//            if (mID.compareTo(this.lastID) == 1) {
//                retrMailIDs.add(mID);
//            }
        }
